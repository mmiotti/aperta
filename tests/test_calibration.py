...
Run with:
    cd src && python -m unittest tests.test_calibration
"""
import unittest
...
def _chain_graph(n: int = 20, edge_len: float = 100.0,
                 speed_kph: float = 36.0, slow_zone_frac: float = 0.4,
...
def _ground_truth_from_model(g: nx.MultiDiGraph, n_trips: int = 200,
    ...
    for _ in range(n_trips):
        ...
        baseline_time = sum(g[u][v][0]['length'] / (g[u][v][0]['speed_kph'] / 3.6)
                            for u, v in edges)
        total_length = sum(g[u][v][0]['length'] for u, v in edges)
        slow_time = sum(g[u][v][0]['length'] * g[u][v][0]['slow_zone']
                        for u, v in edges) / total_length if total_length > 0 else 0
        observed = baseline_time * (1 + slow_zone_coef * slow_time)
        observed += rng.normal(0, noise_std)
        rows.append({
            'orig_x': g.nodes[o_node]['x'], 'orig_y': g.nodes[o_node]['y'],
            'dest_x': g.nodes[d_node]['x'], 'dest_y': g.nodes[d_node]['y'],
...
class CalibrateEdgeWeightsTestCase(unittest.TestCase):
    ...
    def test_recovers_known_multiplier_coefficient(self):
        ...
        """
        g = _chain_graph(n=20)
        gt = _ground_truth_from_model(g, n_trips=400, slow_zone_coef=5.0,
                                      noise_std=2.0, seed=0)
    ...
    def test_returns_expected_result_fields(self):
        ...
        self.assertIn(result.edge_duration_attr,
                      next(iter(g.edges(data=True)))[2])
    ...
    def test_missing_ground_truth_column_raises(self):
        g = _chain_graph(n=5)
        gt = pd.DataFrame({'orig_x': [0], 'orig_y': [0],
                           'dest_x': [400], 'dest_y': [0]})  # no time_measured
    ...
    def test_no_features_runs(self):
        ...
        for _ in range(100):
            ...
            rows.append({
                ...
                'dist_measured': sum(g[u][v][0]['length']
                                     for u, v in zip(path[:-1], path[1:])),
            })
        gt = pd.DataFrame(rows)
        result = calibrate_edge_weights(
            g, gt, constant=False, n_iterations=1,
            snap_max_distance=10.0, min_trip_distance=0.0,
            max_trip_distance=10_000.0, max_dist_to_line_ratio=10.0,
        )
        # α should be very close to 1.0.
        self.assertAlmostEqual(
            result.coefficients.loc['baseline_time', 'coef'], 1.0, places=2)
...
class SnapCountersToEdgesTestCase(unittest.TestCase):
    ...
    def _directed_two_way_graph(self) -> nx.MultiDiGraph:
        ...
        g.add_edge(2, 1, key=0, geometry=LineString(list(line.coords)[::-1]),
                   length=100.0)
        return g
    ...
    def test_directional_counter_picks_correct_edge(self):
        ...
        result = snap_counters_to_edges(counters, g, search_radius=10.0,
    ...
    def test_eligible_edges_filter(self):
        ...
        result = snap_counters_to_edges(
            counters, g, search_radius=10.0,
            eligible_edges=only_motorway)
        self.assertEqual((result.loc[0, 'u'], result.loc[0, 'v']), (1, 2))

    def test_missing_bearing_column_raises(self):
        g = self._directed_two_way_graph()
        counters = gpd.GeoDataFrame(
            geometry=[Point(50.0, 0.0)], crs='EPSG:2056')
...
class EvaluateAgainstCountersTestCase(unittest.TestCase):
    ...
    def test_perfect_match_returns_r2_one_slope_one(self):
        ...
        modeled = pd.Series(
            {(1, 2, 0): 100.0, (2, 3, 0): 200.0,
             (3, 4, 0): 300.0, (4, 5, 0): 400.0})
    ...
    def test_scale_invariant_r2_slope_recovered(self):
        ...
        counters = self._counters([100.0, 200.0, 300.0, 400.0])
        modeled = pd.Series(
            {(1, 2, 0): 200.0, (2, 3, 0): 400.0,
             (3, 4, 0): 600.0, (4, 5, 0): 800.0})
        result = evaluate_against_counters(modeled, counters)
        self.assertAlmostEqual(result['r2'], 1.0)
