...
class AddNodeOverheadsTestCase(unittest.TestCase):
    ...
    def _setup(self):
        pairs = TieredODNodePairs(
            cells_to_cells={"a": np.array(["a", "b", "c"]), "b": np.array(["a", "b"])},
            cells_to_zones={"a": np.array(["ZB"]), "b": np.array(["ZA"])},
            zones_to_zones={"ZA": np.array(["ZB"]), "ZB": np.array(["ZA"])},
        )
        costs = TieredODNodePairs(
            cells_to_cells={"a": np.array([0.0, 10.0, 20.0]), "b": np.array([10.0, 0.0])},
            cells_to_zones={"a": np.array([300.0]), "b": np.array([300.0])},
            zones_to_zones={"ZA": np.array([100.0]), "ZB": np.array([100.0])},
        )
        return pairs, costs
    ...
    def test_all_none_is_no_op(self):
        ...
        np.testing.assert_array_equal(out.cells_to_cells["a"], costs.cells_to_cells["a"])
        np.testing.assert_array_equal(out.zones_to_zones["ZA"], costs.zones_to_zones["ZA"])
    ...
    def test_dict_or_series_accepted(self):
        ...
        b = add_node_overheads(costs, pairs, dest_cell=pd.Series({"a": 1.0, "b": 2.0, "c": 3.0}))
