...
Run with:
    cd src && python -m unittest tests.test_network_processing
"""
import unittest
...
class SnapToNetworkNodesTestCase(unittest.TestCase):
    ...
    def test_eligible_node_ids_filters_targets(self):
        ...
        ids, dists = snap_to_network_nodes(
            points, graph, eligible_node_ids={'b', 'c'},
        )
...
class AggregateEdgesToNodesTestCase(unittest.TestCase):
    ...
    def test_callable_aggregator(self):
        ...
        out = aggregate_edges_to_nodes(
            self._toy_graph(), 'tier', aggregator=lambda arr: float(arr.sum() / 10),
        )
...
class AssignToEligibleCentroidTestCase(unittest.TestCase):
    ...
    def test_snaps_to_median_of_eligible(self):
        ...
        ids, dists = assign_to_eligible_centroid(
            self._polygons(), self._graph_with_tiers(), eligible_node_ids=eligible,
    ...
    def test_mean_centroid(self):
        ...
        ids_mean, _ = assign_to_eligible_centroid(
            self._polygons(), g, eligible_node_ids=eligible, centroid_method='mean',
        )
        ids_med, _ = assign_to_eligible_centroid(
            self._polygons(), g, eligible_node_ids=eligible, centroid_method='median',
        )
    ...
    def test_empty_eligible_raises(self):
        with self.assertRaisesRegex(ValueError, "non-empty"):
            assign_to_eligible_centroid(
                self._polygons(), self._graph_with_tiers(), eligible_node_ids=set(),
            )
...
class FlagNodeIntersectionsTestCase(unittest.TestCase):
    ...
    def _graph(self) -> nx.MultiDiGraph:
        ...
        """
        g = nx.MultiDiGraph()
        for n, x, y in [(1, 0, 0), (2, 1, 0), (3, -1, 0),
                        (4, 0, 1), (5, 0, -1), (6, 2, 0)]:
            g.add_node(n, x=float(x), y=float(y))
        for u, v, hw in [(1, 2, 'primary'),
                         (1, 3, 'residential'),
                         (1, 4, 'residential'),
                         (1, 5, 'residential'),
                         (2, 6, 'residential')]:
            g.add_edge(u, v, highway=hw); g.add_edge(v, u, highway=hw)
        return g
...
class ConsolidateIntersectionsTestCase(unittest.TestCase):
    ...
    def _graph_with_signal_and_roundabout(self) -> nx.MultiDiGraph:
        ...
        g.add_edge(2, 6); g.add_edge(6, 1); g.add_edge(1, 6); g.add_edge(6, 2)
    ...
    def test_signal_reallocated_to_consolidated_node(self):
        ...
        consolidated = consolidate_intersections(
            g, tolerance=20.0, obstacle_buffer=30.0)
        # Find the consolidated central intersection (degree ≥ 4 near 1000,1000).
        central = None
        for nid, d in consolidated.nodes(data=True):
            if (abs(d['x'] - 1000) < 30 and abs(d['y'] - 1000) < 30
                    and d.get('is_degree_4') == 1.0):
                central = nid
                break
    ...
    def test_roundabout_detected_from_edge_tag(self):
        ...
        consolidated = consolidate_intersections(
            g, tolerance=20.0, obstacle_buffer=30.0)
        rb_nodes = [nid for nid, d in consolidated.nodes(data=True)
                    if d.get('is_roundabout') == 1.0]
    ...
    def test_obstacle_buffer_excludes_far_signals(self):
        ...
        consolidated = consolidate_intersections(
            g, tolerance=20.0, obstacle_buffer=2.0)
        any_signal = any(d.get('is_traffic_signal') == 1.0
                         for _, d in consolidated.nodes(data=True))
