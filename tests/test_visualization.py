...
def _toy_cells_zones():
    ...
    cells_geom = [
        box(0, 1, 1, 2),
        box(1, 1, 2, 2),  # top row → Z1
        box(0, 0, 1, 1),
        box(1, 0, 2, 1),  # bottom row → Z2
    ]
    cells = gpd.GeoDataFrame(
        {"zone_id": ["Z1", "Z1", "Z2", "Z2"], "node_id": ["N1", "N2", "N3", "N4"]},
        geometry=cells_geom,
...
class PlotCellValuesTestCase(unittest.TestCase):
    ...
    def test_with_overlays_and_boundary(self):
        overlay = gpd.GeoDataFrame(geometry=[Point(0.5, 0.5), Point(1.5, 1.5)], crs="EPSG:3857")
...
class PlotCellValuesComparisonTestCase(unittest.TestCase):
    ...
    def test_single_panel(self):
        vals = {"only": pd.Series({"C1": 1.0, "C2": 2.0, "C3": 3.0, "C4": 4.0})}
    ...
    def test_shared_scale_uses_global_min_max(self):
        ...
        vals = {
            "low": pd.Series({"C1": 1.0, "C2": 2.0, "C3": 3.0, "C4": 4.0}),
            "high": pd.Series({"C1": 10.0, "C2": 20.0, "C3": 30.0, "C4": 40.0}),
        }
    ...
    def test_independent_scale(self):
        vals = {
            "low": pd.Series({"C1": 1.0, "C2": 2.0, "C3": 3.0, "C4": 4.0}),
            "high": pd.Series({"C1": 100.0, "C2": 200.0, "C3": 300.0, "C4": 400.0}),
        }
...
class PlotTieredDestinationsTestCase(unittest.TestCase):
    ...
    def test_basic_without_graph(self):
        ax = plot_tiered_destinations(self.cells, self.zones, self.pairs, origin_cell_id="C1")
        self.assertIsInstance(ax, plt.Axes)
    ...
    def test_unknown_origin_cell_raises(self):
        with self.assertRaises(KeyError):
            plot_tiered_destinations(self.cells, self.zones, self.pairs, origin_cell_id="C_BOGUS")
...
class PlotEdgeValuesTestCase(unittest.TestCase):
    ...
    def _graph(self) -> nx.MultiDiGraph:
        ...
        for u, v in [(1, 2), (2, 3), (3, 4)]:
            g.add_edge(
                ...
                v,
                key=0,
                geometry=LineString([(g.nodes[u]["x"], 0), (g.nodes[v]["x"], 0)]),
                length=10.0,
                highway="primary",
            )
        return g
    ...
    def test_missing_edges_use_default(self):
        ...
        ax = plot_edge_values(g, values, default_value=0.0, vmin=0.0, vmax=1.0)
        self.assertIsNotNone(ax)
        plt.close("all")
...
class AddStyledColorbarTestCase(unittest.TestCase):
    ...
    def test_returns_cax(self):
        fig, ax = plt.subplots()
        cax = add_styled_colorbar(ax, cmap="viridis", vmin=0, vmax=10, label="test")
