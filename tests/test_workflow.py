...
class WorkflowTestCase(unittest.TestCase):
    ...
    def test_workflow_end_to_end(self):
        ...
        np.testing.assert_array_almost_equal(times.cells_to_zones["a0"], np.array([3.0]))
        # a0 → c0 = a0 → b0 → c0 = 3 + 7 (far tier).
        np.testing.assert_array_almost_equal(times.zones_to_zones["a0"], np.array([10.0]))
        ...
        for origin in df.index:
            ...
            self.assertAlmostEqual(seen, total_pop, msg=f"conservation for origin {origin}")
