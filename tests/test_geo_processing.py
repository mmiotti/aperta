...
Run with:
    cd src && python -m unittest aperta.tests.test_geo_processing
"""
import math
import unittest
...
def _points(coords: list[tuple[float, float]], index: list,
            extra: dict | None = None) -> gpd.GeoDataFrame:
...
class AggregateWithinRadiusTestCase(unittest.TestCase):
    ...
    def test_geoseries_input_works(self):
        ...
        targets = gpd.GeoSeries(
            [Point(0, 0)], index=pd.Index(['T'], name='id'), crs='EPSG:3857')
