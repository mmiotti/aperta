...
def _toy_inputs() -> tuple[TieredODNodePairs, TieredODNodePairs, TieredODNodePairs, dict]:
    ...
    costs = TieredODNodePairs(
        cells_to_cells={"a": np.array([100.0, 200.0, 800.0]), "b": np.array([150.0, 600.0])},
        zones_to_zones={"Z": np.array([1500.0, 5000.0])},
    )
    w_pop = TieredODNodePairs(
        cells_to_cells={"a": np.array([10.0, 20.0, 30.0]), "b": np.array([5.0, 7.0])},
        zones_to_zones={"Z": np.array([100.0, 200.0])},
    )
    w_emp = TieredODNodePairs(
        cells_to_cells={"a": np.array([1.0, 2.0, 3.0]), "b": np.array([4.0, 5.0])},
        zones_to_zones={"Z": np.array([50.0, 60.0])},
    )
    c2z = {"a": "Z", "b": "Z"}
...
class CountInBinsTestCase(unittest.TestCase):
    ...
    def test_three_tiers_all_contribute(self):
        ...
        costs = TieredODNodePairs(
            cells_to_cells={"a": np.array([100.0, 200.0])},
            cells_to_zones={"a": np.array([1500.0])},  # cell origin → zone dest
            zones_to_zones={"Z": np.array([10_000.0])},  # zone origin → zone dest
        )
        w = TieredODNodePairs(
            cells_to_cells={"a": np.array([1.0, 2.0])},
            cells_to_zones={"a": np.array([10.0])},
...
class CountInBinsGeoKeyedTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.costs = TieredODGeoPairs(
            cells_to_cells={
                "c1": np.array([100.0, 200.0, 800.0]),
                "c2": np.array([100.0, 200.0, 800.0]),  # c2 shares c1's outgoing pattern
                "c3": np.array([150.0, 600.0]),
            },
            zones_to_zones={"Z": np.array([1500.0, 5000.0])},
        )
        ...
        self.w_pop = TieredODGeoPairs(
            cells_to_cells={
                ...
                "c2": np.array([10.0, 20.0, 30.0]),
                "c3": np.array([5.0, 7.0]),
            },
            zones_to_zones={"Z": np.array([100.0, 200.0])},
        )
        self.cells_df = pd.DataFrame(
            {"zone_id": ["Z", "Z", "Z"], "walk_overhead_s": [0.0, 100.0, 50.0]},
...
class GravityTestCase(unittest.TestCase):
    ...
    def test_multi_property_amortized_same_result(self):
        ...
        pd.testing.assert_series_equal(single[("d", "pop")], multi[("d", "pop")], check_names=False)

    def test_power_decay_constructor(self):
        """`power_decay` produces `c**(-beta)`. Check against hand computation."""
        beta = 1.0
        costs = TieredODNodePairs(cells_to_cells={"a": np.array([1.0, 2.0, 4.0])})
        w = TieredODNodePairs(cells_to_cells={"a": np.array([1.0, 1.0, 1.0])})
    ...
    def test_custom_decay_callable(self):
        ...
        costs = TieredODNodePairs(cells_to_cells={"a": np.array([0.0, 500.0, 1000.0, 2000.0])})
        w = TieredODNodePairs(cells_to_cells={"a": np.array([1.0, 1.0, 1.0, 1.0])})
    ...
    def test_non_finite_costs_dropped(self):
        ...
        costs = TieredODNodePairs(cells_to_cells={"a": np.array([0.0, np.inf, np.nan, 100.0])})
        w = TieredODNodePairs(cells_to_cells={"a": np.array([1.0, 99.0, 99.0, 1.0])})
    ...
    def test_power_decay_with_zero_cost_does_not_corrupt(self):
        ...
        costs = TieredODNodePairs(cells_to_cells={"a": np.array([0.0, 1.0, 2.0])})
        w = TieredODNodePairs(cells_to_cells={"a": np.array([1.0, 1.0, 1.0])})
    ...
    def test_three_tiers_all_contribute(self):
        ...
        costs = TieredODNodePairs(
            cells_to_cells={"a": np.array([100.0, 200.0])},
            cells_to_zones={"a": np.array([1500.0])},
            zones_to_zones={"Z": np.array([10_000.0])},
        )
        w = TieredODNodePairs(
            cells_to_cells={"a": np.array([1.0, 2.0])},
            cells_to_zones={"a": np.array([10.0])},
    ...
    def test_geo_keyed_overhead_shifts_decay(self):
        ...
        cells_df = pd.DataFrame(
            {"zone_id": ["Z", "Z"], "walk_overhead_s": [0.0, 100.0]},
        ...
        c1_total = (
            ...
            + 20 * np.exp(-beta * 200)
            + 30 * np.exp(-beta * 800)
            + 100 * np.exp(-beta * 1550)
            + 200 * np.exp(-beta * 5050)  # zone-tier +50
        )
        c2_total = (
            10 * np.exp(-beta * 200)
            + 20 * np.exp(-beta * 300)
...
class NearestKTestCase(unittest.TestCase):
    ...
    def test_cost_mean_unit_weights(self):
        ...
        costs = TieredODNodePairs(cells_to_cells={"a": np.array([100.0, 200.0, 300.0, 400.0])})
        w = TieredODNodePairs(cells_to_cells={"a": np.array([1.0, 1.0, 1.0, 1.0])})
    ...
    def test_cost_mean_multi_unit_destination(self):
        ...
        costs = TieredODNodePairs(cells_to_cells={"a": np.array([100.0, 200.0])})
        w = TieredODNodePairs(cells_to_cells={"a": np.array([3.0, 1.0])})
        df = nearest_k(costs, {"w": w}, {"a": None}, ks=[1, 2, 3, 4])
    ...
    def test_cost_mean_fractional_boundary(self):
        ...
        costs = TieredODNodePairs(cells_to_cells={"a": np.array([10.0, 100.0])})
        w = TieredODNodePairs(cells_to_cells={"a": np.array([2.0, 3.0])})
    ...
    def test_k_exceeds_available_returns_nan(self):
        ...
        costs = TieredODNodePairs(cells_to_cells={"a": np.array([100.0, 200.0])})
        w = TieredODNodePairs(cells_to_cells={"a": np.array([1.0, 1.0])})  # total = 2
    ...
    def test_zero_weight_destinations_excluded(self):
        ...
        costs = TieredODNodePairs(cells_to_cells={"a": np.array([100.0, 200.0, 300.0])})
        w = TieredODNodePairs(cells_to_cells={"a": np.array([0.0, 1.0, 1.0])})
    ...
    def test_no_positive_weights_returns_nan(self):
        ...
        costs = TieredODNodePairs(cells_to_cells={"a": np.array([100.0, 200.0])})
        w = TieredODNodePairs(cells_to_cells={"a": np.array([0.0, 0.0])})
    ...
    def test_non_finite_costs_excluded(self):
        ...
        costs = TieredODNodePairs(cells_to_cells={"a": np.array([100.0, np.inf, 200.0, np.nan])})
        w = TieredODNodePairs(cells_to_cells={"a": np.array([1.0, 9.0, 1.0, 9.0])})
        df = nearest_k(costs, {"w": w}, {"a": None}, ks=[1, 2])
    ...
    def test_three_tiers_sort_across(self):
        ...
        costs = TieredODNodePairs(
            cells_to_cells={"a": np.array([2000.0, 100.0])},
            cells_to_zones={"a": np.array([500.0])},  # middle tier (cell origin)
            zones_to_zones={"Z": np.array([1000.0, 10_000.0])},
        )
        w = TieredODNodePairs(
            cells_to_cells={"a": np.array([100.0, 1.0])},
            cells_to_zones={"a": np.array([10.0])},
            zones_to_zones={"Z": np.array([20.0, 1000.0])},
        )
    ...
    def test_geo_keyed_overhead_shifts_cost(self):
        ...
        costs_geo = TieredODGeoPairs(
            ...
            zones_to_zones={
                "Z1": np.array([1500.0, 5000.0]),
                "Z2": np.array([1500.0, 5000.0]),
            },
        )
        pairs_geo = TieredODGeoPairs(
            cells_to_cells={
        ...
        w_pop_geo = TieredODGeoPairs(
            ...
            zones_to_zones={
                "Z1": np.array([100.0, 200.0]),
                "Z2": np.array([100.0, 200.0]),
            },
        )
        cells_df = pd.DataFrame(
            {"zone_id": ["Z1", "Z2"], "walk_overhead_s": [0.0, 100.0]},
            index=pd.Index(["c1", "c2"], name="cell_id"),
        )
        c2z = cells_df["zone_id"].to_dict()
        baked = add_origin_cell_overhead(costs_geo, pairs_geo, cells_df, "walk_overhead_s")
        ...
        np.testing.assert_array_almost_equal(df.loc["c2"].values, df.loc["c1"].values + 100.0)
        self.assertEqual(df.index.name, "cell")
