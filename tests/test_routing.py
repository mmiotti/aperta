...
class SetMinIntrazonalCostTestCase(unittest.TestCase):
    ...
    def _costs(self):
        ...
        return TieredODNodePairs(
            cells_to_cells={"a": np.array([0.0, 5.0, 200.0]), "b": np.array([0.0, 300.0])},
            zones_to_zones={"Z": np.array([1500.0])},
        )
    ...
    def test_scalar_floors_all_entries(self):
        ...
        np.testing.assert_array_equal(out.cells_to_cells["b"], np.array([10.0, 300.0]))
...
class CutoffCorrectnessTestCase(unittest.TestCase):
    ...
    def _graph(self) -> nx.Graph:
        """Same toy graph as TieredPathAggregateTestCase."""
        g = nx.Graph()
        g.add_node("a", x=0.0, y=0.0)
        g.add_node("b", x=1.0, y=0.0)
        g.add_node("c", x=2.0, y=0.0)
