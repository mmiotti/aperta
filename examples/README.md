...
| [`extended/`](extended/) | Near-production-scale, multi-mode showcase on Bern + 25 km: full prep pipeline ([`extended/prepare/`](extended/prepare/)), three-mode accessibility ([`extended/accessibility.ipynb`](extended/accessibility.ipynb)), traffic-flow estimation against observed counters ([`extended/road_stress.ipynb`](extended/road_stress.ipynb)), and edge-weight calibration against ground-truth travel times ([`extended/calibrate_edge_weights.ipynb`](extended/calibrate_edge_weights.ipynb)). | Over an hour for a full run — most of it OSM downloads and network preprocessing in `extended/prepare/`. |
